...
Загружаю данные из **CSV** файла в **DataFrame**. Скачать **CSV** нужно по ссылке - https://disk.yandex.ru/d/7Y7ZQgxUKizQsw <br>
### 2. Обработка данных:

Преобразуйте столбец `date` в формат даты.<br>
Заполняю пропущенные значения, если такие в **csv** файле есть (например, используя средние значения по метеостанциям).<br>
### 3. Анализ данных:
```
 3.1 Определены топ-5 самых жарких дней за все время наблюдений.
 3.2 Выявлена метеостанция с наибольшим количеством осадков за последний год.
 3.3 Рассчитана средняя температура по месяцам за все время наблюдений.
```
### 4. Результаты:<br>
